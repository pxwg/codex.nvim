...
function M.block_under_cursor()
  ...
  if not thread then
    return nil, "Current buffer is not a Coact thread buffer"
  end
  ...
  return nil, "No Coact block under cursor"
end
...
function M.open(block)
  ...
  if not block then
    ...
    util.notify(err or "No Coact block under cursor", vim.log.levels.WARN)
    return
  end
  local bufnr = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_name(bufnr, "coact://detail/" .. block_title(block))
  ...
  end, { buffer = bufnr, silent = true, desc = "Close Coact detail" })
